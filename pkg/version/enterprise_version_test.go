...
entries:
  ...
  - apiVersion: v1
    version: 0.8.1`
		...
		Expect(err).To(BeNil())
		enterpriseVersion, err := version.LatestVersionFromRepo(tmpFile.Name(), "gloo-ee", true)
		Expect(err).To(BeNil())
...
entries:
  ...
  - apiVersion: v1
    version: 0.20.1`
		...
		_, err = tmpFile.WriteString(fileString)
		Expect(err).To(BeNil())
		enterpriseVersion, err := version.LatestVersionFromRepo(tmpFile.Name(), "gloo-ee", false)
		Expect(err).To(BeNil())
